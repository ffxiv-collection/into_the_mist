    ...
    sources.forEach(ms => {
        ...
        div.innerHTML = `
            ...
            <div class="source-info">
                ${sourceTitleHtml}
                ${sourceDetailsHtml}
                ${ms.location ? `<span style="font-weight:bold; font-size:0.9rem;">${ms.location}</span>` : ''}
                ${minion.reputation_rank ? `<span style="font-size:0.9rem;">${minion.reputation_rank}</span>` : ''}
            </div>
            ${costHtml}
        `;
